...
// Helper function to add a column if it doesn't exist in intercepted_data table
...
// Generic helper function to add a column if it doesn't exist in any table
function addColumnIfNotExistsInTable(tableName, columnName, columnDefinition, afterColumn) {
  const dbName = process.env.DB_NAME || "intercepted_data_db";
  const checkQuery = `
    SELECT COUNT(*) AS count 
    FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA = ? 
      ...
      AND COLUMN_NAME = ?;
  `;
  db.query(checkQuery, [dbName, tableName, columnName], (err, results) => {
    if (err) {
      console.error(`Error checking column ${columnName} in ${tableName}:`, err.message);
      return;
    }
    if (results[0].count === 0) {
      const alterQuery = `ALTER TABLE ${tableName} ADD COLUMN ${columnName} ${columnDefinition} AFTER ${afterColumn};`;
      db.query(alterQuery, (err2) => {
        if (err2) {
          console.error(`Error adding column ${columnName} to ${tableName}:`, err2.message);
        } else {
...
// Replies table - updated to support both BBC and IPSO reference numbers
...
db.query(createRepliesTable, (err) => {
  if (err) {
    ...
    // Alter the replies table to add the pdf_file column if it doesn't exist.
    addColumnIfNotExistsInTable("replies", "pdf_file", "VARCHAR(255)", "reply");
  }
});
...
router.post("/replies", upload.single("pdf_file"), (req, res) => {
  ...
  intercept_id = intercept_id.trim();
  bbc_reply = bbc_reply.trim();
  if (bbc_ref_number) {
    bbc_ref_number = bbc_ref_number.trim();
  }

  // Validate TACC Record ID (UUID v4)
  const uuidV4Pattern = /^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-4[0-9a-fA-F]{3}-[89ABab][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$/;
  if (!uuidV4Pattern.test(intercept_id)) {
    return res.status(400).json({ error: "Invalid TACC Record ID format." });
  }

  // Validate the reference number if provided (BBC: AAA-1234567-ABC123 or IPSO: NNNNN-NN)
  const referencePattern = /^(?:[A-Z]{3}-\d{7}-[A-Z0-9]{6}|[0-9]{5}-[0-9]{2})$/;
  if (bbc_ref_number && !referencePattern.test(bbc_ref_number)) {
    return res.status(400).json({ error: "Invalid reference number format." });
  }

  // Ensure reply text is not empty after trimming
  if (bbc_reply.length === 0) {
  ...
  // Check if the provided intercept_id exists in the intercepted_data table
  ...
  db.query(checkInterceptIdQuery, [intercept_id], (err, results) => {
    ...
    // Sanitize reply text and reference number to remove any unwanted HTML
    ...
    db.query(
      insertReplyQuery,
      [sanitizedReference, intercept_id, sanitizedReply, pdfFilePath],
